...
function handleMotionAuthorizationStatus(status) {
  $('#state .motion-authorization-status')
    .find('.value')
    .text(status.toUpperCase())
  if (status === cordova.plugins.diagnostic.motionStatus.NOT_REQUESTED) {
    $('#request-motion').removeAttr('disabled').removeClass('disabled')
  } else {
    $('#request-motion').attr('disabled', 'disabled').addClass('disabled')
  }
}
...
let diagnostic = {
  init: function () {
    console.log(c.cyan + '[init]', 'diagnostic.js')
    $(document).on(
      'deviceready',
      function () {
        this.onDeviceReady()
    ...
    $(document).on(
      ...
      function () {
        ...
        //this.onDeviceReady(options)
      }.bind(this)
    )
  },
  onDeviceReady: function (options) {
    if (options === 'reload')
      console.log(c.cyan + '[reload]', 'diag deviceready')
    else console.log(c.cyan + '[start]', 'diag deviceready')
    ...
    // Bind events
    $(document).on('resume', this.onResume.bind(this))
    // $('#do-check').on('click', this.checkState.bind(this))
    $(document).on('click', '#do-check', this.checkState.bind(this))
    ...
    $('#request-microphone').on('click', function () {
      cordova.plugins.diagnostic.requestMicrophoneAuthorization(function (
        ...
        log(
          'Successfully requested microphone authorization: authorization was ' +
            status
        )
        checkState()
      },
    ...
    $('#request-contacts').on('click', function () {
      cordova.plugins.diagnostic.requestContactsAuthorization(function (
        ...
        log(
          'Successfully requested contacts authorization: authorization was ' +
            status
        )
        checkState()
      },
    ...
    $('#request-calendar').on('click', function () {
      cordova.plugins.diagnostic.requestCalendarAuthorization(function (
        status
      ) {
        log(
          'Successfully requested calendar authorization: authorization was ' +
            status
        )
        checkState()
      },
    ...
    var onLocationRequestChange = function (status) {
      log(
        'Successfully requested location authorization: authorization was ' +
          status
      )
      checkState()
    }
    ...
    $('#request-camera-roll').on('click', function () {
      cordova.plugins.diagnostic.requestCameraRollAuthorization(function (
        ...
        log(
          'Successfully requested camera roll authorization: authorization was ' +
            status
        )
        checkState()
      },
    ...
    $('#request-reminders').on('click', function () {
      cordova.plugins.diagnostic.requestRemindersAuthorization(function (
        status
      ) {
        log(
          'Successfully requested reminders authorization: authorization was ' +
            status
        )
        checkState()
      },
    ...
    $('#get-location').on('click', function () {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        function (err) {
          handleError(
            'Position error: code=' + err.code + '; message=' + err.message
          )
        },
        posOptions
      )
    ...
    //   }.bind(this)
    // )
    setTimeout(this.checkState.bind(this), 1500)
  },
  checkState: function (options) {
    if (options === 'reload')
      console.log(c.cyan + '[reload]', 'Checking state...')
    else console.log('Checking state...')

    $('#state li').removeClass('on off')

    // Location
    var onGetLocationAuthorizationStatus = function (status) {
      $('.request-location').toggle(
    ...
    if (platform === 'android' || platform === 'ios') {
      ...
      cordova.plugins.diagnostic.getLocationAuthorizationStatus(function (
        ...
        $('#state .location-authorization-status')
          .find('.value')
          .text(status.toUpperCase())
    ...
    if (platform === 'android') {
      ...
      cordova.plugins.diagnostic.isNetworkLocationAvailable(function (
        available
      ) {
      ...
      cordova.plugins.diagnostic.hasBluetoothLEPeripheralSupport(function (
        supported
      ) {
        $('#state .bluetooth-le-peripheral-support').addClass(
          supported ? 'on' : 'off'
        )
      },
      handleError)
      ...
      cordova.plugins.diagnostic.isNFCPresent(function (present) {
        ...
        if (!present) {
          $('#nfc-settings').attr('disabled', 'disabled').addClass('disabled')
        }
      }, handleError)
    ...
    // Camera
    var onGetCameraAuthorizationStatus = function (status) {
    ...
    if (platform === 'android' || platform === 'ios') {
      ...
      cordova.plugins.diagnostic.getCameraAuthorizationStatus({
        successCallback: function (status) {
          $('#state .camera-authorization-status')
            .find('.value')
            .text(status.toUpperCase())
    ...
    if (platform === 'ios') {
      ...
      cordova.plugins.diagnostic.getCameraRollAuthorizationStatus(function (
        status
      ) {
        $('#state .camera-roll-authorization-status')
          .find('.value')
          .text(status.toUpperCase())
        $('#request-camera-roll').toggle(
          status === cordova.plugins.diagnostic.permissionStatus.NOT_REQUESTED
        )
      },
      handleError)
    ...
    if (monitoringBluetooth) {
      ...
      if (platform === 'android' || platform === 'ios') {
        cordova.plugins.diagnostic.getBluetoothState(function (state) {
          ...
          $('#request-bluetooth, #monitor-bluetooth').toggle(
            state === cordova.plugins.diagnostic.permissionStatus.DENIED_ALWAYS
          )
        }, handleError)
      }
    }
    ...
    // Microphone
    var onGetMicrophoneAuthorizationStatus = function (status) {
      $('#request-microphone').toggle(
        status === cordova.plugins.diagnostic.permissionStatus.NOT_REQUESTED
      )
    }
    ...
    if (platform === 'android' || platform === 'ios') {
      ...
      cordova.plugins.diagnostic.getMicrophoneAuthorizationStatus(function (
        status
      ) {
        $('#state .microphone-authorization-status')
          .find('.value')
          .text(status.toUpperCase())
    ...
    // Contacts
    var onGetContactsAuthorizationStatus = function (status) {
      $('#request-contacts').toggle(
        status === cordova.plugins.diagnostic.permissionStatus.NOT_REQUESTED
      )
    }
    ...
    if (platform === 'android' || platform === 'ios') {
      ...
      cordova.plugins.diagnostic.getContactsAuthorizationStatus(function (
        status
      ) {
        $('#state .contacts-authorization-status')
          .find('.value')
          .text(status.toUpperCase())
    ...
    // Calendar
    var onGetCalendarAuthorizationStatus = function (status) {
      $('#request-calendar').toggle(
        status === cordova.plugins.diagnostic.permissionStatus.NOT_REQUESTED
      )
    }
    ...
    if (platform === 'android' || platform === 'ios') {
      ...
      cordova.plugins.diagnostic.getCalendarAuthorizationStatus(function (
        status
      ) {
        $('#state .calendar-authorization-status')
          .find('.value')
          .text(status.toUpperCase())
    ...
    if (platform === 'ios' || platform === 'android') {
      ...
      cordova.plugins.diagnostic.isRemoteNotificationsEnabled(function (
        ...
        $('#state .remote-notifications-enabled').addClass(
          enabled ? 'on' : 'off'
        )
      },
      handleError)
    ...
    if (platform === 'ios') {
      ...
      cordova.plugins.diagnostic.getRemindersAuthorizationStatus(function (
        status
      ) {
        $('#state .reminders-authorization-status')
          .find('.value')
          .text(status.toUpperCase())
        $('#request-reminders').toggle(
          status === cordova.plugins.diagnostic.permissionStatus.NOT_REQUESTED
        )
      },
      handleError)
      ...
      // Background refresh
      cordova.plugins.diagnostic.isBackgroundRefreshAuthorized(function (
        enabled
      ) {
        $('#state .background-refresh-authorized').addClass(
          enabled ? 'on' : 'off'
        )
      },
      handleError)
      ...
      cordova.plugins.diagnostic.getBackgroundRefreshStatus(function (status) {
        $('#state .background-refresh-authorization-status')
          .find('.value')
          .text(status.toUpperCase())
      }, handleError)

      // Remote notifications
      var $remoteNotificationsAuthorizationStatusValue = $(
        '#state .remote-notifications-authorization-status'
      ).find('.value')
      if (osVersion >= 10) {
        cordova.plugins.diagnostic.getRemoteNotificationsAuthorizationStatus(
          function (status) {
            $remoteNotificationsAuthorizationStatusValue.text(
              status.toUpperCase()
            )
            $('#request-remote-notifications').toggle(
              status ===
      ...
      cordova.plugins.diagnostic.isRegisteredForRemoteNotifications(function (
        enabled
      ) {
        $('#state .remote-notifications-registered').addClass(
          enabled ? 'on' : 'off'
        )
      },
      handleError)
      ...
      cordova.plugins.diagnostic.isMotionRequestOutcomeAvailable(function (
        available
      ) {
        $('#state .motion-request-outcome-available').addClass(
          available ? 'on' : 'off'
        )
      },
      handleError)
    ...
    if (platform === 'android') {
      ...
      cordova.plugins.diagnostic.getExternalStorageAuthorizationStatus(
        function (status) {
          $('#state .external-sd-authorization-status')
            .find('.value')
            .text(status.toUpperCase())
          $('#request-external-sd-permission').toggle(
            status === cordova.plugins.diagnostic.permissionStatus.NOT_REQUESTED
          )
          $('#request-external-sd-details').toggle(
            status === cordova.plugins.diagnostic.permissionStatus.GRANTED
          )
        },
        handleError
